...
const calculateUserStats = (allEvents: any[], userBetsOnAllEvents: any[]): UserStats => {
    let wins = 0;
    let losses = 0;
...
export function GreetingCard() {
        ...
        const fetchStats = async () => {
            ...
            try {
                ...
                if (allEvents.length > 0) {
                    ...
                    const userBets = await blockchainService.getMultipleUserBets(eventIds, address);
                    const stats = calculateUserStats(allEvents, userBets);
                    setUserStats(stats);
                } else {
                    setUserStats({ wins: 0, losses: 0, totalBets: 0, accuracy: 0, trustScore: 0 });
                }
            } catch (error) {
    ...
    const { tier } = useMemo(() => getTier(userStats?.trustScore ?? 0), [userStats]);
            ...
            <div className="bg-card/60 dark:glass-panel rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 relative overflow-hidden greeting-mesh group">
                ...
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-6">
                        <span className="px-3 py-1 rounded-full border border-primary/30 bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-widest shadow-sm shadow-primary/10">{tier} Tier</span>
