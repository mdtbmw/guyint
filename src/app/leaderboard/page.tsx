...
import { AlertTriangle, Crown, Copy } from "lucide-react";
...
const PodiumCard = ({ user, rank }: { user: LeaderboardUser, rank: number }) => {
        ...
        switch (rank) {
            case 1:
                return {
                    ...
                    mdOrder: "md:order-2",
                    mdTranslateY: "md:-translate-y-10",
                    mdTextSize: "md:text-6xl",
                    mdAvatarSize: "md:w-24 md:h-24",
                    mdPodiumHeight: "md:h-60",
                };
            case 2:
                return {
                    ...
                    mdOrder: "md:order-1",
                    mdTranslateY: "md:-translate-y-0",
                    mdTextSize: "md:text-5xl",
                    mdAvatarSize: "md:w-20 md:h-20",
                    mdPodiumHeight: "md:h-40",
                };
            case 3:
                return {
                    textColor: "text-white",
                    ...
                    mdOrder: "md:order-3",
                    mdTranslateY: "md:-translate-y-0",
                    mdTextSize: "md:text-4xl",
                    mdAvatarSize: "md:w-20 md:h-20",
                    mdPodiumHeight: "md:h-32",
                };
            default:
                return {};
        }
    ...
    return (
         <div className={cn(
             "flex flex-col items-center w-1/3 md:w-1/4 max-w-[200px] text-center transform transition-all duration-500",
             rankStyles.order, rankStyles.zIndex, rankStyles.translateY,
             rankStyles.mdOrder, rankStyles.mdTranslateY,
        )}>
            <div className={cn("rounded-full bg-opacity-20 p-1 border-4 shadow-xl mb-3 animate-float w-20 h-20", rankStyles.borderColor, rankStyles.shadowColor, rankStyles.mdAvatarSize)}>
                <Avatar className="w-full h-full">
                    <AvatarImage src={user.avatar} alt={`Avatar of ${user.username}`} />
                    <AvatarFallback>{user.username.charAt(0)}</AvatarFallback>
                </Avatar>
            </div>
            <p className={cn("text-lg md:text-xl font-display font-bold", rankStyles.textColor)}>{user.username}</p>
            <p className="text-sm text-zinc-500 font-mono">{user.trustScore.toFixed(1)} Score</p>
            <div className={cn(
                "w-full mt-4 rounded-t-xl flex items-center justify-center font-bold shadow-inner shadow-black/50 border-t-4 text-5xl h-40", 
                rankStyles.bgColor, rankStyles.borderColor, rankStyles.rankTextColor,
                rankStyles.mdTextSize, rankStyles.mdPodiumHeight
            )}>
                #{rank}
            </div>
        </div>
        ...
        <div className="space-y-10">
            <div className="animate-slide-up">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                    <PageHeader title="Rankings Protocol" description="Pure Accuracy. Pure Signal."/>
                    <div className="bg-card border border-border p-1 rounded-xl flex gap-1 shadow-lg shrink-0">
                        <button className="px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground text-xs font-bold transition-all hover:bg-secondary">Daily</button>
                        <button className="px-3 py-1.5 rounded-lg text-muted-foreground hover:text-foreground text-xs font-bold transition-all hover:bg-secondary">Weekly</button>
                        <button className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-xs font-bold shadow-sm transition-all border border-primary/30">All-Time</button>
                    </div>
                </div>
            ...
            <div className="animate-slide-up" style={{ animationDelay: '150ms' }}>
                <h2 className="font-display text-2xl font-bold text-foreground mb-8">Signal Masters</h2>
                <div className="relative flex justify-center items-end min-h-[350px] md:min-h-[400px] rounded-t-3xl p-4 md:p-8 podium-deco">
            ...
            <div className="glass-panel rounded-[2.5rem] p-4 md:p-8 animate-slide-up" style={{ animationDelay: "300ms" }}>
                <h2 className="font-display text-xl font-bold text-foreground mb-6 border-b border-border pb-4">
                    Accountability Grid
                </h2>
                                ...
                                return (
                                    <tr key={user.id} className={cn("hover:bg-accent transition-colors", user.id.toLowerCase() === address?.toLowerCase() && "bg-emerald-500/5 dark:bg-emerald-500/10 border-l-4 border-emerald-500/50")}>
                                        ...
                                        <td className="text-foreground flex items-center gap-3">
                                            ...
                                            <span className="font-bold">{isAdmin ? user.id : user.username}</span>
                                            {isAdmin && 
                                                <button onClick={() => handleCopy(user.id)}><Copy className="w-3 h-3 text-muted-foreground"/></button>
                                            }
                                        </td>
