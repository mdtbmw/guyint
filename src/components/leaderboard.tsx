...
import { AlertTriangle, Crown, Award, Medal, RefreshCw, ChevronDown, Copy } from "lucide-react";
...
export function Leaderboard({ limit = 100 }: { limit?: number }) {
    ...
    const [activeTab, setActiveTab] = useState<'accuracy' | 'trustScore'>('trustScore');
    ...
    }, [leaderboardData, activeTab, limit]);
                ...
                {visibleUsers.length > 0 ? visibleUsers.map((user, index) => (
                    <div key={user.id} className={cn("flex items-center gap-4 p-3 rounded-lg border", getRowStyle(index + 1))}>
                        ...
                        <div className="flex-1 overflow-hidden">
                            <p className="font-bold text-sm text-foreground truncate">{isAdmin ? user.id : user.username}</p>
                            <p className="text-muted-foreground text-xs">{getFormattedValue(user)}</p>
                        </div>
                        {isAdmin && (
                            <Button size="icon" variant="ghost" onClick={() => handleCopy(user.id)}>
